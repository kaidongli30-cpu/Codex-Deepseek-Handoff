...
> [!IMPORTANT]
> **このプロジェクトは 2026 年 9 月 21 日にメンテナンスを終了し、アーカイブされました。**
> 最後に正常動作を確認した Codex CLI は `0.151.0-alpha.7.2` です。その後の Codex
> では `config.toml` が書き換えられ、本プロジェクトに必要な管理対象設定が削除されて
> ショートカットが起動できなくなる可能性があります。学習および履歴参照のために
> リポジトリは残しますが、現在の Codex への導入や重要な作業での使用は推奨しません。
